$gp.openai_translate = function( $ ) { return {
	...
	install_hooks: function() {
		$( $gp.openai_translate.table ).on( 'click', 'a.gp_oai_translate', $gp.openai_translate.hooks.openai_translate )
	},
	openai_translate: function( link ) {
		...
		var data = {
			'action': 'gp_oai_translate',
			'query': '',
			'locale': gp_oai_translate.locale,
			'nonce': gp_oai_translate.nonce,
			'original': original_text,
		};

		jQuery.ajax( {
			url: gp_oai_translate.ajaxurl,
			type: 'post',
			data: data,
			datatype: 'json',
		})
